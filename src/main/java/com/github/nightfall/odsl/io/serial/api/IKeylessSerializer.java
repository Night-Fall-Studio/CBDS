...
public interface IKeylessSerializer {
    ...
    static void registerSerializer(IKeylessCustomSerializable<?> serializer) {
        if (INamedSerializer.hasSerializer(serializer.getSerializableType()) && !ODSLConstants.allowSerializerOverwriting) ODSLConstants.LOGGER.warn("Cannot overwrite pre-existing serializers, try turning \"com.github.nightfall.odsl.ODSLConstants.allowSerializerOverwriting\" true.");
    ...
    default void writeCompoundObjectArray(CompoundObject[] array) throws IOException {
        this.writeNamedObjectArray(array);
    }
     ...
     * @param list The list to be written.
     */
    default void writeCompoundObjectList(@NotNull List<CompoundObject> list) throws IOException {
        writeCompoundObjectArray(list.toArray(new CompoundObject[0]));
    }
     ...
     * @param list The list to be written.
     */
    default <T extends INamedSerializable> void writeNamedObjectList(@NotNull List<T> list) throws IOException {
        writeNamedObjectArray(list.toArray(new INamedSerializable[0]));
    }
