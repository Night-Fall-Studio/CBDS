...
public class NamedBinaryDeserializer implements INamedDeserializer {
        ...
        while (true) {
            ...
            switch (type) {
                case STRING_SCHEMA_sBYTE, STRING_SCHEMA_sSHORT, STRING_SCHEMA_sINT -> {
                    ...
                    schema.addAll(List.of(readArray(String[]::new, input::readUTF, STRING_SCHEMA_sBYTE, type)));
                    continue;
                }
                ...
                case STRING -> {
                    if (isUsingStringSchema.get()) {
                        obj = schema.get(input.readInt());
                    }
                    else obj = input.readUTF();
                }
                ...
                case UNNAMED_OBJECT_sBYTE, UNNAMED_OBJECT_sSHORT, UNNAMED_OBJECT_sINT -> obj = newInstance(readArray(Byte[]::new, input::readByte, UNNAMED_OBJECT_sBYTE, type));
    ...
    public CompoundObject readCompoundObject(String name) {
        return this.readNamedObject(CompoundObject.class, name);
    }
    public CompoundObject[] readCompoundObjectArray(String name) {
        return this.readNamedObjectArray(CompoundObject.class, name);
    }
